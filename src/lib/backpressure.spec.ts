...
describe('backpressure', () => {
  ...
  describe('#getStats', () => {
    it('returns current state of the system', async () => {
      const backpressure = new Backpressure(1);
      ...
      const promises = Array.from(Array(3)).map(backpressuredSleeper);
      const stats = backpressure.getStats();
      expect(stats.inflightCalls).toEqual(1);
      expect(stats.pendingCalls).toEqual(2);
